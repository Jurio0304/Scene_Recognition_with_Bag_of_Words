...
def create_results(train_image_paths, test_image_paths, train_labels, test_labels,
                   categories, abbr_categories, predicted_categories, result_path):
    """
    ...
    print('Creating results/Samples.html, thumbnails, and confusion matrix.')
    ...
    files = glob.glob(f'{result_path}/thumbnails/*.jpg')
    for f in files:
        os.remove(f)

    if not os.path.isdir(f'{result_path}/thumbnails'):
        print('Making thumbnails directory.')
        os.mkdir(f'{result_path}/thumbnails')
    ...
    # to recognize.
    with open(f'{result_path}/Samples.html', 'w+') as f:
        ...
        plot_confusion_matrix(confusion_matrix, abbr_categories, title='Normalized confusion matrix')

        # Save the result
        plt.savefig(f'{result_path}/confusion_matrix.png', bbox_inches='tight', dpi=300)

        # Method name
        method = result_path[result_path.index('/') + 1:]
        ...
        f.write('<head>\n');
        f.write(
            '<link href=https://fonts.googleapis.com/css?family=Nunito:300|Crimson+Text|Droid+Sans+Mono'' '
        ...
        f.write('<br>\n');
        f.write('<p style="font-size:30px;">');
        f.write(f'Accuracy (mean of {method}) is {accuracy:.3f}\n');
        f.write('</p>');
        f.write('<p>\n\n');
        ...
        for i, cat in enumerate(categories):
            ...
            for j in range(num_samples):
                if j + 1 <= len(train_examples):
                    ...
                    skimage.io.imsave(f'{result_path}/thumbnails/' + cat + '_' + name, tmp_uint8, quality=100)
    ...
    print('Wrote results page to Samples.html.')
